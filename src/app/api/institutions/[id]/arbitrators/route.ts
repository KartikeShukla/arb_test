...
export async function GET(
  ...
  try {
    ...
    // Only admin or users from the same institution can view arbitrators
    if (
      userData.role !== "admin" &&
      (userData.role !== "institution" || userData.institution_id !== params.id)
    ) {
      return NextResponse.json(
        { error: "Not authorized to view these arbitrators" },
        { status: 403 },
      );
    }

    // Get arbitrators for the institution
    const { data: arbitrators, error: arbitratorsError } = await supabase
      .from("users")
    ...
    if (arbitratorsError) {
      return NextResponse.json(
        { error: arbitratorsError.message },
        { status: 500 },
      );
    }
